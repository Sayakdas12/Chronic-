...
async function saveLocation(location) {
  state.locations = [...new Set([...state.locations, location])].slice(-5);
  localStorage.setItem("riskWatchLocations", JSON.stringify(state.locations));
  if (!auth.currentUser) return;
  try {
    const locationsRef = ref(database, `riskWatch/${auth.currentUser.uid}/locations`);
    await set(push(locationsRef), { name: location, alertsEnabled: false, createdAt: new Date().toISOString() });
    const remoteLocations = await get(locationsRef);
    const entries = remoteLocations.exists() ? Object.entries(remoteLocations.val()).sort((first, second) => String(first[1].createdAt || "").localeCompare(String(second[1].createdAt || ""))) : [];
    await Promise.all(entries.slice(0, -5).map(([id]) => remove(ref(database, `riskWatch/${auth.currentUser.uid}/locations/${id}`))));
  } catch (error) {
    console.warn("Could not sync monitored location to Firebase:", error.message);
    toast("Location saved on this device; cloud sync is unavailable.");
  }
}
function preserveCurrentLocation() {
  if (state.location && !state.locations.includes(state.location)) {
    state.locations = [...state.locations, state.location].slice(-5);
    localStorage.setItem("riskWatchLocations", JSON.stringify(state.locations));
  }
}
$("locationForm").addEventListener("submit", (event) => { event.preventDefault(); const location = $("locationInput").value.trim(); if (!location) return; preserveCurrentLocation(); void saveLocation(location); $("locationInput").value = ""; void setActiveLocation(location); toast(`Now monitoring ${location}`); }); $("locateButton").addEventListener("click", () => { if (locationRequestInProgress) return; if (!navigator.geolocation) { setLocationStatus("Location is unavailable. Search manually instead.", "error"); return; } locationRequestInProgress = true; $("locateButton").disabled = true; setLocationStatus("Checking your location... Getting your current location, please wait.", "loading"); navigator.geolocation.getCurrentPosition(async (position) => { preserveCurrentLocation(); const coordinates = { latitude: position.coords.latitude, longitude: position.coords.longitude }; localStorage.setItem("riskWatchLocations", JSON.stringify(state.locations)); try { await setActiveLocation("Current location", coordinates); setLocationStatus("Location updated successfully", "success"); } catch (error) { setLocationStatus("Location detected, but live data could not be loaded. Please retry.", "error"); } finally { locationRequestInProgress = false; $("locateButton").disabled = false; } }, () => { locationRequestInProgress = false; $("locateButton").disabled = false; setLocationStatus("Location permission was denied or detection failed. Please retry or search manually.", "error"); }); }); $("refreshButton").addEventListener("click", load); $("addLocationButton").addEventListener("click", () => $("locationInput").focus());
$("savedLocations").addEventListener("click", async (event) => {
  const button = event.target.closest("[data-remove]");
  if (button) {
    state.locations = state.locations.filter((item) => item !== button.dataset.remove).slice(-5);
    localStorage.setItem("riskWatchLocations", JSON.stringify(state.locations));
    renderLocations();
    toast("Location removed");
    return;
  }
  const row = event.target.closest(".saved");
  const selected = row?.querySelector("b")?.textContent?.trim();
  if (!selected || selected === state.location) return;
  await setActiveLocation(selected);
  toast(`Showing live data for ${selected}`);
}); $("radiusSelect").value = String(state.radiusKm); $("radiusSelect").addEventListener("change", () => { state.radiusKm = Number($("radiusSelect").value); localStorage.setItem("riskWatchRadiusKm", String(state.radiusKm)); clearLocationData(); void load(); }); $("situationMap").addEventListener("click", (event) => { const marker = event.target.closest("[data-signal-index]"); const signal = marker && state.snapshot?.situations?.[Number(marker.dataset.signalIndex)]; if (signal) toast(`${signal.type}: ${signal.severity}, ${signal.distanceKm} km away. Source: ${signal.source}`); }); $("nearbySignals").addEventListener("click", (event) => { const card = event.target.closest("[data-signal-index]"); const signal = card && state.snapshot?.situations?.[Number(card.dataset.signalIndex)]; if (signal) toast(`${signal.type}: ${signal.risk}`); }); $("checklist").addEventListener("change", (event) => { const index = Number(event.target.dataset.check); state.checked = event.target.checked ? [...state.checked, index] : state.checked.filter((item) => item !== index); localStorage.setItem("riskWatchChecklist", JSON.stringify(state.checked)); renderChecklist(); }); $("warningDetails").addEventListener("click", () => toast("Avoid floodwater and follow official instructions.")); $("notifyButton").addEventListener("click", async () => { if (!("Notification" in window)) return toast("Browser notifications unavailable"); toast((await Notification.requestPermission()) === "granted" ? "Browser notifications enabled" : "Notifications remain off"); }); $("locationInput").addEventListener("keydown", (event) => { if (event.key === "Enter") $("locationForm").requestSubmit(); }); renderLocations(); renderChecklist(); load(); refreshTimer = setInterval(load, 60000); window.addEventListener("beforeunload", () => clearInterval(refreshTimer));
